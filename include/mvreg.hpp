...
template <actor_type A, value_type T>
struct mvreg {
    struct value {
    ...
    void reset_remove(const version_vector<A>& clock) {
        std::ranges::transform(vals, begin(vals),
                [clock = &clock] (const auto& val) { val.vclock.reset_remove(clock); });
    ...
    auto write(add_context<A>&& ctx, T val) -> value {
    ...
    auto clock() -> version_vector<A> {
        ...
        for (const auto& val: vals) resutl.merge(val.vclock);
        return resutl;
    }
};
