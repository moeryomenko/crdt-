...
template <actor_type A> struct Arbitrary<dot<A>> {
    ...
    {
        return gen::build<dot<A>>(gen::set(&dot<A>::actor), gen::set(&dot<A>::counter, gen::inRange(0, 100)));
    }
};
}
...
{
    assert(rc::check(
        "inc increments only the counter", [](dot<int> d) { RC_ASSERT(dot(d.actor, d.counter + 1) == ++d); }));
