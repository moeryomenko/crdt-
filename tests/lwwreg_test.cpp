...
auto is_conflicted_marker(lwwreg<V, M> &reg1, lwwreg<V, M> &reg2) -> bool {
...
auto main() -> int {
  ...
  assert(rc::check("associative", [](pair r1, pair r2, pair r3) {
    ...
    auto has_confliciting_marker = is_conflicted_marker(reg1, reg2) ||
                                   is_conflicted_marker(reg1, reg3) ||
                                   is_conflicted_marker(reg2, reg3);
  ...
  assert(rc::check("change delta", [](int old, int val) {
    ...
    auto delta = replica1.update(actor, val);
